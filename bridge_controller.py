...
class BridgeController(QObject):
    ...
    def _attempt_ws_reconnect(self) -> None:
        ...
        def _check_and_reconnect():
            ...
            try:
                ...
                self._apply_version_info(version_info)
                if self._bridge_version_mismatch_detected():
                    log.warning("Bridge version mismatch on reconnect (bridge=%s/%s server=%s/%s) — stopping", updater_service.normalize_version(BRIDGE_VERSION), updater_service.normalize_version(BRIDGE_COMPONENT_VERSION), updater_service.normalize_version(self._server_version), self._server_bridge_component_version or "<none>")
                    self._ws_intentional_disconnect = True
                    self.bridge_version_mismatch.emit(self._build_version_payload("bridge", version_info.get("bridge_download_url", "")))
                    return
            except Exception as e:
    ...
    def _push_chat_hotkey_to_game(self) -> None:
        self.udp.send_to_game({
            "event": "chat_hotkey",
            "key_code": settings_store.get_chat_hotkey_code(),
            "key_label": settings_store.get_chat_hotkey_label(),
        })

    def update_chat_hotkey_setting(self, key_code: str, key_label: str) -> None:
        settings_store.set_chat_hotkey(key_code, key_label)
        if self.udp.is_game_connected():
            self.udp.send_to_game({
                "event": "chat_hotkey",
                "key_code": key_code,
    ...
    def _on_game_connected(self) -> None:
        ...
        self._push_chat_hotkey_to_game()
