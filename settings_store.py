...
def get_overlay_color() -> str:
    return _load().get("overlay_color", DEFAULT_OVERLAY_COLOR)
...
def get_chat_hotkey_code() -> str:
    return _load().get("chat_hotkey_code", "F8")


def get_chat_hotkey_label() -> str:
    return _load().get("chat_hotkey_label", "F8")


def set_chat_hotkey(code: str, label: str) -> None:
    data = _load()
    data["chat_hotkey_code"] = code
    data["chat_hotkey_label"] = label
    _save(data)
