...
_SIDEBAR_BTN = (
    f"QPushButton {{ text-align: center; padding: 10px 16px; border: none; border-radius: 4px; color: {CLR_TEXT}; font-size: 16px; font-weight: bold; }} "
...
class MainWindow(QMainWindow):
    def __init__(self, controller):
        ...
        self._update_btn_row = self._build_update_button_row()
        sidebar_layout.addWidget(self._update_btn_row)
        ...
        self._detail_page.set_back_callback(lambda: self._stack.setCurrentIndex(self._HISTORY_IDX))
    ...
    def _build_update_button_row(self) -> QWidget:
        row = QWidget()
        ...
        self._update_dot.setStyleSheet("color: #ff4d4f; font-size: 16px; font-weight: bold;")
        self._update_dot.setVisible(False)
        layout.addWidget(self._update_dot, alignment=Qt.AlignVCenter)
    ...
    def _create_update_page(self) -> QWidget:
        ...
        sub = QLabel("Keep your bridge and game mod in sync.")
        sub.setStyleSheet(f"color: {CLR_TEXT_BRIGHT}; font-size: 16px; font-weight: bold;")
        ...
        self._update_panel = UpdatePanel(
            self._controller,
            title="",
            subtitle="Keep your bridge and game mod in sync.",
        )
    ...
    def _create_match_mode_page(self) -> QWidget:
        ...
        label.setStyleSheet(f"color: {CLR_ACTIVE_BTN}; font-size: 30px; font-weight: bold;")
    ...
    def _on_ws_disconnected(self):
        if self._stack.currentIndex() in (self._LOGIN_IDX, self._VERSION_MISMATCH_IDX):
            return
        self._sidebar.setVisible(False)
    ...
    def _update_overlay_btn_style(self):
        if self._overlay.isVisible():
            self._overlay_btn.setStyleSheet(
                f"QPushButton {{ text-align: center; padding: 10px 16px; border: none; border-radius: 4px; color: white; font-size: 16px; font-weight: bold; background-color: {CLR_ACTIVE_BTN}; }}"
            )
        else:
            self._overlay_btn.setStyleSheet(_SIDEBAR_BTN)
    ...
    def _check_update_indicator(self):
        try:
            ...
            if hasattr(self._update_panel, "_bridge_version_value"):
                local_bridge = self._update_panel._bridge_version_value.text().strip()
            if hasattr(self._update_panel, "_server_version_value"):
                latest_bridge = self._update_panel._server_version_value.text().strip()
